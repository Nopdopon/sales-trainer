...
# ФИКС БАГА №1: точное имя модели подтверждено из вкладки "Код" в Playground —
# там показывается реальное тело curl-запроса с полем "model", которое
# Playground отправляет в API при выбранной модели в выпадающем списке.
# Это самый надёжный источник, так как это буквально тот же запрос,
# который делает рабочий Playground.
