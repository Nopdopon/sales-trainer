...
def screen_menu():
    ...
    st.caption("Тренажёр переговоров для менеджеров по продажам. Выберите тему, тип звонка и клиента.")
    ...
    for col, persona in zip(cols, filtered):
        with col:
            ...
            if persona.avatar_url:
                avatar_html = f'<img src="{persona.avatar_url}" style="width:72px;height:72px;border-radius:50%;object-fit:cover;display:block;margin:0 auto 8px auto;">'
            else:
                avatar_html = f'<div style="font-size:56px;text-align:center;margin-bottom:8px;">{persona.emoji}</div>'
            ...
            <div class="persona-card">
                ...
                <div style="text-align:center;margin:4px 0 6px 0;">
                    ...
                    <span style="font-size:11px;color:#6b7280;margin-left:6px;">
                        {CALL_TYPE_ICON.get(persona.call_type,'')} {persona.call_type}
                    </span>
                </div>
            ...
            if st.button(f"▶️ Начать с {persona.name}", key=f"start_{persona.key}",
